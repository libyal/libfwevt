...
then
	docker run -t -v "${PWD}:/libfwevt" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libfwevt && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libfwevt;

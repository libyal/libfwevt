...
LIBRARY_TESTS="channel error event keyword level manifest map opcode provider support task template xml_document xml_tag xml_template_value xml_token";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfwevt" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
